        ...
        <div class="overflow-x-auto">
            <table id="budjetTable" class="table-auto min-w-full divide-y divide-gray-200 bg-white shadow overflow-hidden rounded-lg">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">{{$jurie->id}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$jurie->numJury}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$jurie->matiere}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$jurie->nbrCopies}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$jurie->created_at}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button onclick="toggleButtons('actions-{{$jurie->id}}')" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Show</button>
                            <div id="actions-{{$jurie->id}}" class="hidden mt-2 flex space-x-2">
                                <form action="{{route('edite-jurie',$jurie->id)}}" method="get">
                                    @csrf
                                    <button class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                                </form>
                                <form action="{{route('delete-jurie',$jurie->id)}}" method="post">
                                    @method('DELETE')
                                    @csrf
                                    <button class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                                </form>
                                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600"><a href="{{route('show-jurie',$jurie->id)}}">Display</a></button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
