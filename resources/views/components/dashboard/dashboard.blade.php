            ...
            <table id="budjetTable" class="table-auto min-w-full divide-y divide-gray-200 bg-white shadow overflow-hidden rounded-lg">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">jury </th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">taux</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">nbrCopie</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">prixCopie</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">dateCreation</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @foreach ($details as $dashboard)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->person}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->jury}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->taux}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->nbrCopies}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->prixCopie}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$dashboard->dateCorrection}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button onclick="toggleButtons('actions-{{$dashboard->id}}')" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Show</button>
                            <div id="actions-{{$dashboard->id}}" class="hidden mt-2 flex space-x-2">
                                <form action="{{route('edite-dashboard',$dashboard->id)}}" method="get">
                                    @csrf
                                    <button class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                                </form>
                                <form action="{{route('delete-dashboard',$dashboard->id)}}" method="post">
                                    @method('DELETE')
                                    @csrf
                                    <button class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                                </form>
                                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600"><a href="{{route('show-dashboard',$dashboard->id)}}">Display</a></button>
                                <button class="px-3 py-1 bg-gray-100 text-black rounded hover:bg-gray-300"><a href="{{route('dashboard-pdf')}}">export</a></button>
