...
<body class="bg-gray-100">
    ...
    <div class="section max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="overflow-x-auto">
            <table class="table-auto min-w-full divide-y divide-gray-200 bg-white shadow overflow-hidden rounded-lg">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DDRP</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">{{$personne->id}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$personne->DDRP}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$personne->nomFr}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$personne->nomAr}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$personne->created_at}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button onclick="toggleButtons('actions-{{$personne->id}}')"
                                class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Display</button>
                            <div id="actions-{{$personne->id}}" class="hidden mt-2 flex space-x-2">
                                <form action="{{route('edite-personne',$personne->id)}}" method="get">
                                    @csrf
                                ...
                                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600"><a href="{{route('show-personne',$personne->id)}}">Show</a></button>
                                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600"><a href="{{route('personne-pdf')}}">export</a></button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
