    ...
    <input type="checkbox" id="check">
            ...
            <div class="sidbar">
                ...
                <ul class="bg-gray-900">
                    <li><a href="/partials"><i class="fas fa-qrcode"></i>Dashboard</a></li>
        ...
        <div class="overflow-x-auto">
            <table class="table-auto min-w-full divide-y divide-gray-200 bg-white shadow overflow-hidden rounded-lg">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">{{$etab->id}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$etab->Code}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$etab->NomFr}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$etab->NomAr}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$etab->created_at}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button onclick="toggleButtons('actions-{{$etab->id}}')"
                                class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Display</button>
                            <div id="actions-{{$etab->id}}" class="hidden mt-2 flex space-x-2">
                                <form action="{{route('edite-etablissement',$etab->id)}}" method="get">
                                    @csrf
                                    <button class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                                </form>
                                <form action="{{route('delete-etablissement',$etab->id)}}" method="post">
                                    @method('DELETE')
                                    @csrf
                                    <button class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                                </form>
                                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600"><a href="{{route('show-etablissement',$etab->id)}}">Show</a></button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
